...
class HttpServer {
  ...
  run() {
    var self = this;
    mongoose.connect(self.url);     // connect to mongoDB database on modulus.io

    app.use(express.static(self.path + '/public'));               // set the static files location /public/img will be /img for users
  ...
  defineAPI() {
    var FatCats = mongoose.model('feeding', {datetime : {type: Date, required: true}});

    //GET
    app.get('/api/feedings', function(req, res){
      FatCats.find().sort({_id:-1}).exec(function(err, feedings) {
        if (err) {
          res.send(err);
        }
        res.json(feedings);
